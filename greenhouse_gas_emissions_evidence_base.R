...
# new colours - four inc NS
weca_core_colours <- read_json(
...
source_data_tbl <- read_filter(source_path, woe_las = woe_las)
...
start_value <- historic_tbl$emissions_co2e_kt[
  historic_tbl$calendar_year == start_year
]
end_value <- historic_tbl$emissions_co2e_kt[
  historic_tbl$calendar_year == end_year
]
...
ggsave(
  'plots/WoE_emissions_net_zero_2023_inc_ns_new_colours.png',
  plot = plot,
  device = "png",
  height = 7,
  width = 10
...
plot_tbl %>%
  write_csv('data/woe_emissions_territorial_2023_inc_ns.csv')
...
sector_change_10yr_tbl <- sector_tbl %>%
  filter(
    calendar_year == min_year |
      calendar_year == cur_year
  ) %>%
  group_by(la_ghg_sector, calendar_year) %>%
  summarise(
    emissions_co2e_kt = sum(territorial_emissions_kt_co2e),
    .groups = "drop"
  ) %>%
  pivot_wider(
    id_cols = la_ghg_sector,
    names_from = calendar_year,
    values_from = emissions_co2e_kt,
    names_prefix = "year_"
  ) %>%
  set_names(new_names) %>%
  glimpse()
...
chart_tbl <- sector_change_10yr_tbl %>%
  mutate(
    percentage_of_total = (year_end / sum(year_end)) * 100,
    change_10yr_perc = ((year_start - year_end) * -100 / year_start)
  ) %>%
  arrange(change_10yr_perc) %>%
  mutate(
    axis_name = glue("{la_ghg_sector} ({round(percentage_of_total)}%)") %>%
      as_factor()
  ) %>%
  glimpse()

sector_change_plot <- ggplot(
  chart_tbl,
  aes(x = axis_name, y = change_10yr_perc)
) +
  geom_segment(
    aes(x = axis_name, xend = axis_name, y = 0, yend = change_10yr_perc),
    color = "gray",
    lwd = 2
  ) +
  geom_point(inherit.aes = TRUE, size = 4, pch = 21, bg = 4, col = 1) +
  scale_x_discrete(labels = chart_tbl$axis_name) +
  coord_flip() +
  labs(
    title = glue("Emissions change by sector\n{min_year} to {cur_year}"),
    y = "Percentage change",
...
ggsave(
  "plots/sector_change_plot_inc_ns.png",
...
sector_pop_tbl <- subset_co2_tbl |>
  ...
  summarise(
    total_emissions_woe = sum(total_emissions, na.rm = TRUE),
...
gt_source_tbl_1 <- sector_pop_tbl %>%
  ...
  pivot_wider(
    id_cols = la_ghg_sector,
    names_from = calendar_year,
    values_from = total_emissions_woe,
    names_prefix = "year_"
  ) %>%
  set_names(new_names) %>%
  mutate(
    pct_change_10yr = (year_end - year_start) / year_start,
...
gt_source_tbl <- gt_source_tbl_1 |>
  ...
  mutate(
    la_ghg_sector = if_else(
      la_ghg_sector == "LULUCF",
      "Land Use",
      la_ghg_sector
    )
  ) |>
...
woe_broad_sector_tbl <- sector_tbl |>
  group_by(la_ghg_sector, calendar_year) |>
  summarise(
    total_emissions = sum(territorial_emissions_kt_co2e),
    .groups = "drop"
  ) |>
  pivot_wider(names_from = la_ghg_sector, values_from = total_emissions) |>
  mutate(
    `Business and Public` = Agriculture +
      Commercial +
      Industry +
      `Public Sector` +
      LULUCF +
...
sector_3_time_series_plot <- woe_broad_sector_tbl %>%
  ...
  labs(
    title = "Greenhouse Gases: Territorial Emissions Trends",
...
ggsave(
  "plots/sector_3_time_series_plot.png",
...
per_cap_emissions_tbl <- woe_sector_tbl %>%
  ...
  summarise(
    emissions_la_kt = sum(territorial_emissions_kt_co2e),
    population = mean(mid_year_population_thousands),
    .groups = "drop"
  ) %>%
  group_by(calendar_year) %>%
  summarise(
    total_emissions_woe = sum(emissions_la_kt),
...
decline_per_cap_territorial <- ((per_cap_emissions_tbl$per_cap_emissions[
  per_cap_emissions_tbl$calendar_year == start_year
] -
  per_cap_emissions_tbl$per_cap_emissions[
    per_cap_emissions_tbl$calendar_year == cur_year
  ]) /
  per_cap_emissions_tbl$per_cap_emissions[
    per_cap_emissions_tbl$calendar_year == start_year
  ]) *
...
ind_com_chart <- woe_sector_pop_tbl %>%
  ...
  scale_x_continuous(
    labels = unique(woe_sector_pop_tbl$calendar_year),
    breaks = unique(woe_sector_pop_tbl$calendar_year)
  ) +
  scale_color_manual(
    values = c("Industry" = "#ed749d", "Commercial" = "#6bd4f2")
  ) +
  geom_line(lwd = 2) +
  labs(
    title = "Greenhouse Gases: Emissions Trends",
...
ggsave(
  "plots/commercial_and_industrial_trends_woe.png",
  plot = ind_com_chart,
  bg = "white",
  height = 7,
  width = 10
...
la_breakdown_tbl <- woe_sector_tbl |>
  ...
  summarise(
    total_emissions = sum(territorial_emissions_kt_co2e),
    total_emissions_la_control = sum(
      emissions_within_the_scope_of_influence_of_l_as_kt_co2
    )
  ) |>
  pivot_longer(
    cols = c(total_emissions, total_emissions_la_control),
    names_to = "emission_type",
    values_to = "KtCO2e"
  ) |>
  mutate(
    emission_type = if_else(
      emission_type == "total_emissions",
      "Total Emissions",
...
plot_la_bar_10yr <- la_breakdown_tbl %>%
  ...
  chart_theme() +
  labs(
    title = "Territorial Emissions by Local Authority",
    subtitle = "West of England",
    x = "Year",
    y = quickText("KtCO2e"),
...
ggsave(
  "plots/la_plot_10yr_total_territorial_emissions.png",
  plot_la_bar_10yr,
  bg = "white",
  height = 7,
  width = 10
...
la_breakdown_tbl |>
  filter(
    calendar_year == cur_year | calendar_year == min_year,
  ...
  pivot_wider(
    ...
    names_from = calendar_year,
    values_from = KtCO2e,
    names_prefix = "Territorial_Emissions_KtCO2e_"
  ) |>
  mutate(
    pct_change_10yr = round(
      ((`Territorial_Emissions_KtCO2e_2022` -
        `Territorial_Emissions_KtCO2e_2013`) /
        `Territorial_Emissions_KtCO2e_2013`) *
        100
    )
  ) |>
...
source_data_tbl |>
  ...
  mutate(
    percentage_of_total = total_emissions *
      100 /
      sum(total_emissions, na.rm = TRUE)
  )
